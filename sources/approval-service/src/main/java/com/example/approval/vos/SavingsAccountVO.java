...
public class SavingsAccountVO {
	...
	public SavingsAccountVO(HashMap<String, String> payload, String opType) throws Exception{
    	...
    	if( key == null ) {
        	if( (this.opType == OperationType.updateSA) || (this.opType == OperationType.createSA) ) {
        		this.mandatoryValidationMessage = this.mandatoryValidationMessage + "Userid is not provided. ";
        	}
    		setUserid("");
    	}else {
    		setUserid(key);
	...
	public void setValues(ResultSet rs) throws Exception{
		...
		try {
			this.setApproveOrRejectDate(rs.getString("approve_or_reject_date") != null ? rs.getString("approve_or_reject_date").substring(0, 10) : "");
		} catch (SQLException e) {
			// Continue.. this field may not be retrieved from database
		}
		try {
